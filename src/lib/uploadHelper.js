...
async function logUploadStep(step, message, metadata = {}, accessToken = null) {
    try {
        ...
        if (accessToken) {
            ...
            try {
                const { data: { session: cachedSession } } = await Promise.race([
                    supabase.auth.getSession(),
...
export async function uploadFileRobust(bucket, path, file, toast = null, accessToken = null) {
    ...
    return new Promise(async (resolve) => {
        try {
            ...
            logUploadStep('preflight', `Starting upload: ${file.name} (${(file.size / 1024).toFixed(1)}KB)`, logMeta, accessToken);
            ...
            if (file.size > 10 * 1024 * 1024) {
                logUploadStep('error', `File too large: ${(file.size / 1024 / 1024).toFixed(1)}MB`, logMeta, accessToken);
            ...
            if (!token) {
                ...
                if (sessionErr || !session) {
                    ...
                    return resolve({ data: null, error: sessionErr || new Error('Not authenticated. Please log in again.') });
                }
            ...
            logUploadStep('uploading', `XHR upload starting: ${kbSize}KB to ${bucket}/${path}`, { ...logMeta, kbSize }, accessToken);
            ...
            xhr.onload = () => {
                if (xhr.status >= 200 && xhr.status < 300) {
                    ...
                    logUploadStep('success', `Upload complete! HTTP ${xhr.status}`, { ...logMeta, httpStatus: xhr.status }, accessToken);
                    ...
                    logUploadStep('error', errMsg, { ...logMeta, httpStatus: xhr.status, response: xhr.responseText?.substring(0, 500) }, accessToken);
            ...
            xhr.onerror = () => {
                ...
                logUploadStep('network_error', 'XHR network error — connection lost or CORS issue', logMeta, accessToken);
            ...
            logUploadStep('error', `Unexpected error: ${err.message}`, { ...logMeta, stack: err.stack?.substring(0, 500) }, accessToken);
