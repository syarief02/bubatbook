...
export default function CreateCustomer() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const { error: icUpErr } = await uploadFileRobust(
        'customer-documents',
        icPath,
        icFile,
        toast
      ...
      const { error: licUpErr } = await uploadFileRobust(
        'customer-documents',
        licPath,
        licenceFile,
        toast
      ...
      <div className="glass-card max-w-2xl">
        <div className="flex items-center gap-2 mb-6">
          ...
          <p className="text-xs text-green-400">
            Customer will be auto-verified — admin has validated documents.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="input-label flex items-center gap-1.5">
                <CreditCard className="w-3.5 h-3.5 text-violet-400" /> IC Number *
              </label>
              <input
                value={form.ic_number}
                onChange={set('ic_number')}
                className="input-field"
                placeholder="901234-14-5678"
              />
              {errors.ic_number && (
                <p className="text-[10px] text-red-400 mt-1">{errors.ic_number}</p>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            {errors.address_line1 && (
              <p className="text-[10px] text-red-400 mt-1">{errors.address_line1}</p>
            )}
            <input
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="input-label flex items-center gap-1.5">
                <Calendar className="w-3.5 h-3.5 text-violet-400" /> Licence Expiry *
              </label>
              <input
                type="date"
              ...
              {errors.licence_expiry && (
                <p className="text-[10px] text-red-400 mt-1">{errors.licence_expiry}</p>
              )}
            </div>
            <div>
              <label className="input-label">License Type</label>
              <select
                value={form.gdl_license}
                onChange={set('gdl_license')}
                className="input-field"
              >
                {GDL_OPTIONS.map((o) => (
